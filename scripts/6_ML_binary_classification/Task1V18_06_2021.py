...
if(useAACompo == True):
    ...
    print("AA Composition option ON\n")
# In[12]:
...
if(useAACompo == True):
    hotEncodedKeysExternal = np.sum(hotEncodedKeysExternal, axis = 2)
...
for repeat in range(nRepeats):
	...
	if useAACompo == True:
		real_condition = real_condition + 10
	
	file_object = open('History.txt', 'a')
	file_object.write(antigenID + "\t" + str(nEpochs) + "\t" + str(nNeurons) + "\t" + str(nSeqTot) + "\t" + str(repeat) + "\t" + str(nRepeats)+ "\t" + str(real_condition) + "\t" + str(batch_size) + "\t" + str(layers) + '\t' + str(model.evaluate(test_dataset)) + '\t' + str(roc_auc) + '\t' + str(model.evaluate(train_dataset)) + "\t" + str(model.evaluate(external_dataset)) + "\n")
	file_object.close()
