...
if(runningInCommandLine):
    ...
    if(len(sys.argv) > 10):
        ...
        print("Output column assign to be ", colOutput)

    if(len(sys.argv) > 11):
        simSuffix = sys.argv[11]
        print("Suffix assign to be ", simSuffix)

    if(len(sys.argv) > 12):
        FileExternal = sys.argv[12]
        print("External test file assign to be ", simSuffix)
...
if(FileExternal != ""):
    # We need to hope and assume that the words are the same for both files
    (paddedInputTensorExternal, paddedOutputTensorExternal, word2idxINExt, idx2wordINExt, word2idxOUTExt, idx2wordOUTExt, NIExt, NOExt) = processFile(FileExternal, wordSizeIn, wordSizeOut, colInput, colOutput, paddingInputs, paddingOutputs)

    if(not word2idxINExt == word2idxIN):
        print("ERR: different vocabulary IN for test")

    if(not idx2wordINExt == idx2wordIN):
        print("ERR: different vocabulary for test")

    if(not word2idxOUTExt == word2idxOUT):
        print("ERR: different vocabulary for test")

    if(not idx2wordOUTExt == idx2wordOUT):
        print("ERR: different vocabulary for test")

    if(not(NIExt == NI)):
        print("ERR: different vocabulary input size for test")
...
if(FileExternal != ""):
    nExternal = len(paddedInputTensorExternal)
    nSeqTotExternal = min(nSeqTot,nExternal)
...
# In[ ]:
	...
	externalIDs = [*range(0, nSeqTotExternal)] 
	random.shuffle(externalIDs)
	...
	if(FileExternal != ""):
		externalKeys = paddedInputTensorExternal[externalIDs[0:nSeqTotExternal]].astype(float);	
	...
	if(FileExternal != ""):
		externalLabels = paddedOutputTensorExternal[externalIDs[0:nSeqTotExternal]].astype(float);
	...
	if(FileExternal != ""):
		list_batched_inputs = np.array_split(externalKeys, 100, 0)
		list_batched_outputs =  np.array_split(externalLabels, 100, 0)

		if(len(externalKeys) > 100000):
			list_batched_inputs = np.array_split(externalKeys[0:100000], 100, 0)
			list_batched_outputs =  np.array_split(externalLabels[0:100000], 100, 0)
		...
		for index in range(0,100):
			...
			AllResults = []
			f = open(simCode + "/" + "ExternalResults.txt", "a")
			if(index == 0):
				f.write("lenout\tlenpred\tlenin\tld\tldnorm\tpred\tout\tin\n")
