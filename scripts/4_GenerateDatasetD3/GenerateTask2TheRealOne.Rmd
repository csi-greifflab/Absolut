...
# Part 1 : does the job on slices data 
...
for(antigenID in lib){
    print(antigenID)
    fileBalanced <- paste(folder, antigenID, "_Task1",prefix,"_SlicesBalancedData.txt", sep="")
...
```{r}
print(paste("Full Database task 2", prefix, " has", length(df$Slide), "sequences"))
print(paste("Full Database task 2", prefix, " has", length(unique(df$Slide)), "sequences"))
...
```{r}
write.table(groupedDfForOutput, paste("Task2", prefix, "Annotated.txt", sep=""), sep="\t", quote=FALSE, row.names=FALSE)
```
...
```{r}
write.table(forSaving, paste("Task2", prefix, "BindAtLeastOne.txt", sep=""), quote=FALSE, sep="\t", row.names=FALSE) 
```




# Part 2 : same job on CDR3 data 
...
for(antigenID in lib){
    print(antigenID)
    fileBalanced <- paste(folder, antigenID, "_Task1",prefix,"_BalancedData.txt", sep="")
    a <- read.table(fileBalanced, header=TRUE)
    b <- a %>% mutate(Class = ifelse(Label == "Binder", antigenID, Label))
    df <- rbind(df, b)
}
...
```{r}
print(paste("Full Database task 2", prefix, " has", length(df$CDR3), "sequences"))
print(paste("Full Database task 2", prefix, " has", length(unique(df$CDR3)), "sequences"))
...
```{r}
groupedDfForOutput <- df %>% group_by(CDR3, Label) %>% mutate(Duplicates = n(), nDiffAntigens = n_distinct(Class)) %>% arrange(CDR3) %>% select(CDR3, Label, Duplicates,  nDiffAntigens, Class)
groupedDfForOutput
```


```{r}
write.table(groupedDfForOutput, paste("Task2", prefix, "CDR3Annotated.txt", sep=""), sep="\t", quote=FALSE, row.names=FALSE)
```
...
```{r}
groupedDf <- df %>% group_by(CDR3, Label) %>% mutate(Duplicates = n(), nDiffAntigens = n_distinct(Class)) %>% slice(1) %>% arrange(CDR3) %>% select(CDR3, Label, Duplicates,  nDiffAntigens, Class)
groupedDf
```
...
```{r}
write.table(forSaving, paste("Task2", prefix, "CDR3BindAtLeastOne.txt", sep=""), quote=FALSE, sep="\t", row.names=FALSE) 
```