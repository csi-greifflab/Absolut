...
def lr_trainer_nsamples_ntimes_t4(infile):
    '''
    train an svm model, uses phil's data, uses adapted encoder and binarizer from phil, uses phil's preproessing
    uses the multiclass variant of the classifier.
    :param infile:
    ...
    nsamples = [100, 2000, 10000, 50000, 250000]
    #nsamples = [100, 2000, 10000, 50000]
    #nsamples = [10000,50000]
    #nsamples = [item for item in nsamples if item < nrows]
    print(nsamples)
    outcontents = []
    ntimes=10
    outseqdf = pd.DataFrame()
    for nsample in nsamples[-1:]:
        outns = []
        for i in range(ntimes):
            X_train, y_train, X_test, y_test, x_train, x_test = pdt(infile, nsample)
            print(X_train.shape, y_train.shape)
        ...
        print(outtag)
        outname = 'clf_acc_antigens_outfiles_t4/%s_lr_acc_nsamples_nrep%s_pscheme.csv' % (outtag, ntimes)
        outname_seq = 'clf_acc_antigens_outfiles_t4/%s_lr_rawoutput_nsamples_nrep%s_pscheme.csv' % (outtag,ntimes)
        outdf.to_csv(outname, index=False)
        outseqdf.to_csv(outname_seq, index=False)
...
def lr_trainer_nsamples_ntimes_t4_shuffled(infile):
    '''
    train an svm model, uses phil's data, uses adapted encoder and binarizer from phil, uses phil's preproessing
    uses the multiclass variant of the classifier.
    :param infile:
    ...
    for nsample in nsamples[-1:]:
        ...
        outtag = infile.split('/')[-1].split('.')[0]
        print(outtag)
        outname = 'clf_acc_antigens_outfiles_t4/%s_lr_acc_nsamples_nrep%s_pscheme_shuffled.csv' % (outtag, ntimes)
        outname_seq = 'clf_acc_antigens_outfiles_t4/%s_lr_rawoutput_nsamples_nrep%s_pscheme_shuffled.csv' % (outtag,ntimes)
        outdf.to_csv(outname, index=False)
        outseqdf.to_csv(outname_seq, index=False)
...
def lr_trainer_nsamples_ntimes_t4_batch(infile):
    '''
    train an svm model, uses phil's data, uses adapted encoder and binarizer from phil, uses phil's preproessing
    uses the multiclass variant of the classifier.
    :param infile:
    ...
    for nsample in nsamples[:]:
        ...
        for i in range(ntimes):
            ...
            print(X_train)
            print(X_test)
            labels = list(set(np.concatenate([y_train, y_test])))
            #print(labels, len(labels))
            print('stop here')
            # clf = SVC(kernel='linear')
            ### incremental learning
            training_batches = batch (X_train, y_train, n=10000)
            for i, (X_train_batch, y_train_batch) in enumerate(training_batches):
                clf = SGDClassifier(loss='log')
                print('batches starts here')
                clf.partial_fit(X_train_batch,y_train_batch, labels)
                print(X_train_batch.shape, y_train_batch.shape, 'train_batches', infile)
        ...
        outtag = infile.split('/')[-1].split('.')[0]
        print(outtag)
        outname = 'clf_acc_antigens_outfiles_t4/%s_lr_acc_nsamples%s_nrep%s_pscheme_batch.csv' % (outtag,nsample, ntimes)
        outname_seq = 'clf_acc_antigens_outfiles_t4/%s_lr_rawoutput_samples%s_nrep%s_pscheme.csv' % (outtag, nsample, i)
        outdf.to_csv(outname, index=False)
        #outseqdf.to_csv(outname_seq, index=False)


def lr_trainer_nsamples_ntimes_t4_batch_shuffled(infile):
    '''
    train an svm model, uses phil's data, uses adapted encoder and binarizer from phil, uses phil's preproessing
    uses the multiclass variant of the classifier.
    :param infile:
    ...
    for nsample in nsamples[:]:
        ...
        outtag = infile.split('/')[-1].split('.')[0]
        print(outtag)
        outname = 'clf_acc_antigens_outfiles_t4/%s_lr_acc_nsamples%s_nrep%s_pscheme_batch_shuffled.csv' % (outtag,nsample, ntimes)
        outname_seq = 'clf_acc_antigens_outfiles_t4/%s_lr_rawoutput_samples%s_nrep%s_pscheme.csv' % (outtag, nsample, i)
        outdf.to_csv(outname, index=False)
        #outseqdf.to_csv(outname_seq, index=False)
...
#sorted_infiles = [item for item in sorted_infiles if 'D2' not in item and 'Seq' in item][:1]
#sorted_infiles = [item for item in sorted_infiles if 'D2' in item and 'Seq' in item][:1]

# limit to motif seqD2 since this file failed to compute
#sorted_infiles = [item for item in sorted_infiles if 'Motif' not in item]
...
infile = data
print(data)
#lr_trainer_nsamples_ntimes_t4(infile)
#lr_trainer_nsamples_ntimes_t4_shuffled(infile)
#lr_trainer_nsamples_ntimes_t4_batch(infile)
lr_trainer_nsamples_ntimes_t4_batch_shuffled(infile)
