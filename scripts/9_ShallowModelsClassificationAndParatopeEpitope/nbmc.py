...
def nb_trainer_nsamples_ntimes_nantigen(infile):
    '''
    ...
    for nantigen in nantigens[:]:
        for nsample in nsamples[:]:
            ...
            for i in range(ntimes)[:]:
                ...
                ntrain  = len(X_train)
                ntest = len(X_test)
                outcontents.append([confs_str, s, nrep, 'nb', ntrain, ntest, nantigen])
    outcols = ['conf_m', 'accuracy', 'nrep', 'clf', 'ntrain', 'ntest', 'nantigen']
    outdf = pd.DataFrame(outcontents, columns=outcols)
    print(outdf)
    outname = 'clf_acc_antigens_outfiles/t2_nb_cm_acc_nsamples_nantigens_nrep%s_pscheme.csv' % ntimes
    print(outname)
    outdf.to_csv(outname, index=False)


def nb_trainer_nsamples_ntimes_nantigen_shuffled(infile):
    '''
    train an svm model, uses phil's data, uses adapted encoder and binarizer from phil, uses phil's preproessing
    uses the multiclass variant of the classifier.
    :param infile:
    ...
    outdf = pd.DataFrame(outcontents, columns=outcols)
    print(outdf)
    outname = 'clf_acc_antigens_outfiles/t2_nb_cm_acc_nsamples_nantigens_nrep%s_pscheme_shuffled.csv' % ntimes
    print(outname)
    outdf.to_csv(outname, index=False)
...
def nb_trainer_nsamples_ntimes_nantigen_shuffled_aacomp(infile):
    '''
    train an svm model, uses phil's data, uses adapted encoder and binarizer from phil, uses phil's preproessing
    uses the multiclass variant of the classifier.
    :param infile:
    ...
    outdf = pd.DataFrame(outcontents, columns=outcols)
    print(outdf)
    outname = 'clf_acc_antigens_outfiles/t2_nb_cm_acc_nsamples_nantigens_nrep%s_pscheme_aacomp_shuffled.csv' % ntimes
    print(outname)
    outdf.to_csv(outname, index=False)


def nb_trainer_nsamples_ntimes_nantigen_aacomp(infile):
    '''
    train an svm model, uses phil's data, uses adapted encoder and binarizer from phil, uses phil's preproessing
    uses the multiclass variant of the classifier.
    :param infile:
    ...
    outdf = pd.DataFrame(outcontents, columns=outcols)
    print(outdf)
    outname = 'clf_acc_antigens_outfiles/t2_nb_cm_acc_nsamples_nantigens_nrep%s_pscheme_aacomp.csv' % ntimes
    print(outname)
    outdf.to_csv(outname, index=False)
...
#nb_trainer_nsamples_ntimes_nantigen('dataset/Treated142.txt')
#nb_trainer_nsamples_ntimes_nantigen_shuffled('dataset/Treated142.txt')
#nb_trainer_nsamples_ntimes_nantigen_shuffled_aacomp('dataset/Treated142.txt')
#nb_trainer_nsamples_ntimes_nantigen_aacomp('dataset/Treated142.txt')
