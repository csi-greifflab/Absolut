...
def lr_trainer_nsamples_ntimes_antigen2(infile):
    '''
    train an svm model, uses phil's data, uses adapted encoder and binarizer from phil
    ...
    for nsample in nsamples[:]:
        ...
        for i in range(ntimes):
            ...
            outns.append(s)
            nrep = 'rep%s' % (i + 1)
            outcontents.append([tn, fp, fn, tp, s, nrep, 'lr', antigen, ntrain,ntest])
        #mean_s = np.mean(outns)
        #sd_s = np.std(outns)
        #print(mean_s, sd_s)
    ...
    outdf = pd.DataFrame(outcontents, columns=outcols)
    #outname = 'outfiles/lr_acc_nsamples_n%s.csv' % ntimes
    outname = 'clf_acc_antigens_outfiles/%s_lr_cm_acc_nsamples_nrep%s_pscheme.csv' % (antigen, ntimes)
    outdf.to_csv(outname, index=False)
...
for infile in infiles:
    lr_trainer_nsamples_ntimes_antigen2(infile)
