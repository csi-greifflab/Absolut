...
def svm_trainer_nsamples_ntimes_t4(infile):
    '''
    train an svm model, uses phil's data, uses adapted encoder and binarizer from phil, uses phil's preproessing
    uses the multiclass variant of the classifier.
    :param infile:
    ...
    for nsample in nsamples[:]:
        ...
        print(outtag)
        outname = 'clf_acc_antigens_outfiles_t4/%s_svm_acc_nsamples_nrep%s_pscheme.csv' % (outtag, ntimes)
        outname_seq = 'clf_acc_antigens_outfiles_t4/%s_svm_rawoutput_nsamples_nrep%s_pscheme.csv' % (outtag,ntimes)
        outdf.to_csv(outname, index=False)
        outseqdf.to_csv(outname_seq, index=False)



def svm_trainer_nsamples_ntimes_t4_shuffled(infile):
    '''
    train an svm model, uses phil's data, uses adapted encoder and binarizer from phil, uses phil's preproessing
    uses the multiclass variant of the classifier.
    :param infile:
    ...
    for nsample in nsamples[:]:
        ...
        outtag = infile.split('/')[-1].split('.')[0]
        print(outtag)
        outname = 'clf_acc_antigens_outfiles_t4/%s_svm_acc_nsamples_nrep%s_pscheme_shuffled.csv' % (outtag, ntimes)
        outname_seq = 'clf_acc_antigens_outfiles_t4/%s_svm_rawoutput_nsamples_nrep%s_pscheme_shuffled.csv' % (outtag,ntimes)
        outdf.to_csv(outname, index=False)
        outseqdf.to_csv(outname_seq, index=False)
...
#    svm_trainer_nsamples_ntimes_t4(infile)
#    sys.exit()
comm = MPI.COMM_WORLD
size = comm.Get_size()
rank = comm.Get_rank()
...
print(data)
#svm_trainer_nsamples_ntimes_t4(infile)
svm_trainer_nsamples_ntimes_t4_shuffled(infile)
